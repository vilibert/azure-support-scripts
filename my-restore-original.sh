...
# Get teh data disk name which is the copy of the os-disk
osdisk_copy=$(az vm show -g $rg -n $rn | jq ".storageProfile.dataDisks[0].name" | sed s/\"//g )



# Detach the Problematic OS disk from the Rescue VM
echo "Detaching the OS disk from the rescue VM"
az vm disk detach -g $rg --vm-name $rn -n $osdisk_copy
...
swap=$(az vm update -g $g -n $vm --os-disk $osdisk_copy)
#if [[ $(echo "${swap//\"}") != "$osdisk" ]]
#then
